    ...
    SitePaymentGatewayAttributeValue.transaction do
      begin
        params_hash[:payment_gateway_attribute].each do |payment_attribute, payment_value|
          ...
          site_payment_gateway_attribute_value = find_or_initialize_by_site_payment_gateway_id_and_payment_gateway_attribute_id(site_payment_gateway.id, payment_gateway_attribute.id)
